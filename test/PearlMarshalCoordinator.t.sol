...
contract PearlMarshalCoordinatorTest is Test {
    PearlMarshalCoordinator private controller;
    ...
    bytes32 private actionId = keccak256("synthesis_olas");
    bytes32 private subjectId = keccak256("subject");

    function setUp() public {
        controller = new PearlMarshalCoordinator(admin, operator, reporter, 100 ether);
        vm.startPrank(admin);
    ...
    function testExecuteBoundedActionRevertsWithoutReportedBalance() public {
        PearlMarshalCoordinator freshController = new PearlMarshalCoordinator(admin, operator, reporter, 100 ether);
        vm.startPrank(admin);
