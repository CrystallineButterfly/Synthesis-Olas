...
MODULE = importlib.import_module('agents.pearl_marshal')
...
class ProjectContextTest(unittest.TestCase):
    ...
    def test_primary_names_match(self) -> None:
        spec = MODULE.build_project_spec()
        self.assertEqual(spec.primary_python_module, 'pearl_marshal')
        self.assertEqual(spec.primary_contract_name, 'PearlMarshalCoordinator')
